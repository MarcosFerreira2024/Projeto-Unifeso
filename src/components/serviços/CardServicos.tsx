...
function CardServicos({titulo,url="/servicos/agenda.svg",alt,height=81,width=81,data}:card) {

    const {openModal} = React.useContext(ContextService)

  return (
    <div role='button' onClick={(event) => openModal(event)} data-descricao={data as string}  className='min-h-[250px] hover:opacity-80  sm:min-h-[350px] min-w-[250px] sm:min-w-[350px] max-h-[250px] sm:max-h-[350px] max-w-[250px] sm:max-w-[350px]  mx-auto flex-1 hover:border-[4px]  hover:border-[#f3f3f3] border-[4px] border-[transparent] flex-wrap rounded-[20px] flex justify-around flex-col cardservicos bg-white'>
